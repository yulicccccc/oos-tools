...
if st.session_state.report_generated:
    with st.spinner("Compiling Celsis logic..."):
        ...
        if t_suite == "L-Suite":
            ...
            airflow_text = f"The {sample_noun} was processed within the ISO 5 Biological Safety Cabinet (BSC), E00{st.session_state.bsc_id}, situated in the innermost cleanroom ({t_suite}B), which connects to the intermediate ISO 7 buffer room ({t_suite}A). This buffer room, in turn, leads to the outermost ISO 8 anteroom ({t_suite}). Furthermore, a positive air pressure system is maintained between the suites, ensuring airflow from the innermost ISO 7 cleanroom to the intermediate ISO 7 buffer room and finally to the outermost ISO 8 anteroom."
            suite_inner = f"cleanroom {t_suite}B"
            disinf_route = f"the ISO 8 anteroom ({t_suite}), where the vials underwent a second disinfection using acidified bleach, again with a 10-minute contact time. Subsequently, the vials were moved into the ISO 7 cleanroom ({t_suite}B)."
            suite_str_display = f"{t_suite} and 114" if t_suite != "114" else "114"
            p11 = f"Monthly cleaning and disinfection of the outermost ISO 8 anteroom, the middle ISO 7 buffer room, the innermost ISO 7 cleanroom, and its containing ISO 5 Biosafety Cabinets for {suite_str_display} was performed on {st.session_state.monthly_cleaning_date}. Cleaning was performed as per SOP 2.600.018 (Cleaning and Disinfecting Procedure for Microbiology). Following the monthly cleaning, it was documented that all H₂O₂ indicators passed. thus, confirming efficient monthly cleaning of all three parts of {suite_str_display}."

        p4 = airflow_text
        p5 = f"The innermost ISO 7 {suite_inner}, and the ISO 5 BSC E00{st.session_state.bsc_id} within the {suite_inner} were thoroughly cleaned and prepared before initiating the testing by analyst {a_init} as per SOP 2.600.002 (Environmental Monitoring of the Clean Room Facility), and SOP 2.600.018 (Cleaning and Disinfecting Procedure for Microbiology). Similarly, for Celsis aliquoting, the ISO 7 cleanroom 114A, and the ISO 5 BSC E001798 within 114A were thoroughly cleaned and prepared by aliquoting analyst {alq_init}, before initiating the testing as per SOP 2.600.002 and SOP 2.600.018. Both the BSCs, E00{st.session_state.bsc_id} and E001798 in 114A, were certified and approved by the Engineering and Quality Assurance teams prior to use."
        ...
        p7 = f"Upon completion of incubation on {st.session_state.test_date}, TSB & FTM bottles for {st.session_state.sample_id} were disinfected and transferred to the middle ISO 7 buffer room (114A) for aliquoting step per SOP 2.600.059 (Celsis Sterility Testing). In 114A, the media bottles were disinfected one more time before transferring them to the ISO 5 BSC E001798 located in 114A. In ISO 5 BSC E001798, the {sample_noun} was aliquoted into assay cuvettes by analyst {alq_init}."
        ...
        word_data = {
            ...
            "smart_scan_id": f"E00{st.session_state.celsis_id}", "smart_cr_id": f"E00{t_room} (L-Suite)" if t_suite == "L-Suite" else (f"E00{t_room} ({t_suite})" if t_suite == "114" else f"For Processing/Reading: E00{t_room} ({t_suite})\nFor Aliquoting: E001736 (114)"),
