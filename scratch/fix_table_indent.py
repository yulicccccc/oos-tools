...
# --- 1. RE-GENERATE WORD DOCX (LEFT ALIGNED, ZERO INDENT) ---
doc = Document()
...
# Set column widths strictly matching page width (6.5 inches total)
col_widths = [1.3, 1.9, 2.1, 1.2]
table = doc.add_table(rows=len(table_data), cols=4)

# STRICTLY LEFT ALIGN TABLE AND ZERO INDENT
...
doc.save(docx_out)
print(f"Updated Word table with ZERO indent: {docx_out}")

# --- 2. UPDATE HTML FILE (MARGIN-LEFT: 0, EXACT LEFT ALIGN) ---
html_rows = ""
for r_idx, row in enumerate(table_data[1:]):
    ...
    <tr>
      ...
      <td style="border: 1px solid #B0B0B0; padding: 6px 8px; vertical-align: top;"><b>{rev_html}</b></td>
...
<p style="margin: 12px 0 6px 0;">@Simin Mohammad please review and update the OOS form text fields accordingly. You may directly use the newly compiled 8-page PDF package attached here.</p>
...
with open(html_out, "w", encoding="utf-8") as f:
    f.write(full_html)
print(f"Updated clean HTML: {html_out}")

# --- 3. RE-COPY TO CLIPBOARD WITH ZERO INDENT ---
HEADER = """Version:0.9
...
try:
    ...
    win32clipboard.SetClipboardText("OOS-261186 Review Email Table (Left Aligned)")
    print("SUCCESS: Copied perfectly left-aligned HTML table to Clipboard!")
finally:
